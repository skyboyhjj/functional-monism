#!/usr/bin/env bash
# 环境准备：Python 依赖（markdown2 / weasyprint / fonttools）+ MathJax（node）
set -e
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "[1/2] Python 依赖 markdown2 / weasyprint / fonttools ..."
python3 -m pip install --quiet markdown2 weasyprint fonttools
